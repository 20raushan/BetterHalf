...
<?php 
       $find_email = $conn->query("SELECT  user_id FROM betterhalf_user where serial_no = '$ser_no'");
       $find_email_res = $find_email->fetch_array(MYSQLI_BOTH);
       $email_of_sender= $find_email_res['user_id'];
       $email_of_receiver = $_SESSION['email_id'];
       $check_st_req = $conn->query("select * from request where senderid='$email_of_sender' and receiverid = '$email_of_receiver' and request_status='Pending' ");
$check_st_res = $check_st_req->fetch_array(MYSQLI_BOTH);
if($check_st_res)
{?>
